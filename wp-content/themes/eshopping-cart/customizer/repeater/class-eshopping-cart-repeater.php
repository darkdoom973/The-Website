...
/**
 * Class EShopping_Cart_Repeater
 */
class EShopping_Cart_Repeater extends WP_Customize_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->add_field_label = esc_html__( 'Add new field', 'eshopping-cart' );
		if ( ! empty( $args['add_field_label'] ) ) {
			$this->add_field_label = $args['add_field_label'];
		}

		$this->boxtitle = esc_html__( 'Customizer Repeater', 'eshopping-cart' );
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'eshopping_cart_customizer-repeater-script', get_template_directory_uri() . '/customizer/repeater/script.js', array( 'jquery', 'jquery-ui-draggable', 'wp-color-picker' ), '1.0.0', true );

		wp_enqueue_script( 'eshopping_cart_customizer-repeater-fontawesome-iconpicker', get_template_directory_uri() . '/customizer/repeater/icon-picker/icon-picker.js', array( 'jquery' ), '1.0.0', true );
				...
				<div class="customizer-repeater-general-control-repeater-container customizer-repeater-draggable">
					...
					<div class="customizer-repeater-box-content-hidden">
						...
						if ( $this->customizer_repeater_color_control == true ) {
							$this->input_control(
								array(
									'label'             => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Color', 'eshopping-cart' ), $this->id, 'customizer_repeater_color_control' ),
						...
						if ( $this->customizer_repeater_color2_control == true ) {
							$this->input_control(
								array(
									'label'             => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Color', 'eshopping-cart' ), $this->id, 'customizer_repeater_color2_control' ),
						...
						if ( $this->customizer_repeater_title_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Title', 'eshopping-cart' ), $this->id, 'customizer_repeater_title_control' ),
...
///////////// price///////////
						if ( $this->customizer_repeater_price_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Price', 'eshopping-cart' ), $this->id, 'customizer_repeater_price_control' ),
...
///////////// number///////////
						if ( $this->customizer_repeater_number_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Number', 'eshopping-cart' ), $this->id, 'customizer_repeater_number_control' ),
						...
						if ( $this->customizer_repeater_subtitle_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Subtitle', 'eshopping-cart' ), $this->id, 'customizer_repeater_subtitle_control' ),
						...
						if ( $this->customizer_repeater_text_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Text', 'eshopping-cart' ), $this->id, 'customizer_repeater_text_control' ),
						...
						if ( $this->customizer_repeater_link_control ) {
							$this->input_control(
								array(
									'label'             => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Link', 'eshopping-cart' ), $this->id, 'customizer_repeater_link_control' ),
						...
						if ( $this->customizer_repeater_text2_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Button Text', 'eshopping-cart' ), $this->id, 'customizer_repeater_text2_control' ),
						...
						if ( $this->customizer_repeater_link2_control ) {
							$this->input_control(
								array(
									'label'             => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Link', 'eshopping-cart' ), $this->id, 'customizer_repeater_link2_control' ),
						...
						if ( $this->customizer_repeater_shortcode_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Shortcode', 'eshopping-cart' ), $this->id, 'customizer_repeater_shortcode_control' ),
						...
						echo '>';
						esc_html_e( 'Delete field', 'eshopping-cart' );
						?>
						</button>
...
//number control//////////
					...
					<button type="button" class="social-repeater-general-control-remove-field button" style="display:none;">
						<?php esc_html_e( 'Delete field', 'eshopping-cart' ); ?>
					</button>
				</div>
			</div>
			<?php
	...
	private function icon_picker_control( $value = '', $show = '' ) {
		...
		?>
			>
			<span class="customize-control-title">
				<?php esc_html_e( 'Icon', 'eshopping-cart' ); ?>
			</span>
			<span class="description customize-control-description">
				<?php
				echo sprintf(
					/* translators: Fontawesome link with full list of icons available */
					esc_html__( 'Note: Some icons may not be displayed here. You can see the full list of icons at Fontawesome Website.', 'eshopping-cart' )
	...
	private function image_control( $value = '', $show = '' ) {
		...
		?>
			>
			<span class="customize-control-title">
				<?php esc_html_e( 'Image', 'eshopping-cart' ); ?>
			</span>
			<input type="text" class="widefat custom-media-url" value="<?php echo esc_attr( $value ); ?>">
			<input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_attr_e( 'Upload Image', 'eshopping-cart' ); ?>" />
		</div>
		<?php
	}
	...
	private function logo_image_control( $value = '', $show = '' ) {
		...
		?>
			>
			<span class="customize-control-title">
				<?php esc_html_e( 'Logo Image', 'eshopping-cart' ); ?>
			</span>
			<input type="text" class="widefat custom-logo-media-url" value="<?php echo esc_attr( $value ); ?>">
			<input type="button" class="button button-secondary customizer-repeater-logo-custom-media-button" value="<?php esc_attr_e( 'Upload Image', 'eshopping-cart' ); ?>" />
		</div>
		<?php
	}
	...
	private function icon_type_choice( $value = 'customizer_repeater_icon' ) {
		?>
		<span class="customize-control-title">
			<?php esc_html_e( 'Image type', 'eshopping-cart' ); ?>
		</span>
		<select class="customizer-repeater-image-choice">
			<option value="customizer_repeater_icon" <?php selected( $value, 'customizer_repeater_icon' ); ?>><?php esc_html_e( 'Icon', 'eshopping-cart' ); ?></option>
			<option value="customizer_repeater_image" <?php selected( $value, 'customizer_repeater_image' ); ?>><?php esc_html_e( 'Image', 'eshopping-cart' ); ?></option>
			<option value="customizer_repeater_logo_image" <?php selected( $value, 'customizer_repeater_logo_image' ); ?>><?php esc_html_e( 'Logo Image', 'eshopping-cart' ); ?></option>
			<option value="customizer_repeater_none" <?php selected( $value, 'customizer_repeater_none' ); ?>><?php esc_html_e( 'None', 'eshopping-cart' ); ?></option>
		</select>
		<?php
	}
	...
	private function repeater_control( $value = '' ) {
		...
		$show_del        = 0;
		?>
		<span class="customize-control-title"><?php esc_html_e( 'Social icons', 'eshopping-cart' ); ?></span>
		<?php
		...
		if ( ( count( $social_repeater ) == 1 && '' === $social_repeater[0] ) || empty( $social_repeater ) ) {
			...
			<div class="customizer-repeater-social-repeater">
				<div class="customizer-repeater-social-repeater-container">
					...
					<?php get_template_part( $this->customizer_icon_container ); ?>
					<input type="text" class="customizer-repeater-social-repeater-link" placeholder="<?php esc_attr_e( 'Link', 'eshopping-cart' ); ?>">
					<input type="hidden" class="customizer-repeater-social-repeater-id" value="">
					<button class="social-repeater-remove-social-item" style="display:none">
						<?php esc_html_e( 'Remove Icon', 'eshopping-cart' ); ?>
					</button>
				</div>
				<input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value=""/>
			</div>
			<button class="social-repeater-add-social-item button-secondary"><?php esc_html_e( 'Add Icon', 'eshopping-cart' ); ?></button>
			<?php
		} else {
			?>
			<div class="customizer-repeater-social-repeater">
				...
				foreach ( $social_repeater as $social_icon ) {
					...
					if ( ! empty( $social_icon['icon'] ) ) {
						...
						echo '<input type="text" class="customizer-repeater-social-repeater-link" placeholder="' . esc_attr__( 'Link', 'eshopping-cart' ) . '" value="';
					...
					if ( $show_del == 1 ) {
						...
						echo '">' . esc_html__( 'Remove Icon', 'eshopping-cart' ) . '</button>';
					echo '</div>';
				}
				?>
